 ...
 $holidays = DB::table('holidays')->whereRAW('YEAR(holiday_date) = "'.$year.'" ')
    ...
    */
    ...
    public function viewPosts(Request $request)  {
        if (Auth::check()) { 
            $limit = 50;  $page_no = 0;
            $categories = Category::where('status', 'ACTIVE')->orderby('position', 'asc')->get(); 
    ...
    public function editPostSms(Request $request)
    {
        if (Auth::check()) {
            ...
            $get_category=Category::where('status','ACTIVE')->select('id','name')->orderBy('position',"ASC")->get();
    ...
    public function viewContactsList()
    {
        if (Auth::check()) {
            ...
            return view('admin.contacts_list');
        } else {
            return redirect('/admin/login');
        }
    }
    ...
    public function getContactsList(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            $status = $request->get('status',0);
           if($status != ''){
            $clist = ContactsList::where('status','=',$status)->where('school_id', $school_id)->get();
           }else{
            $clist = ContactsList::where('school_id', $school_id)->get();
           } 
