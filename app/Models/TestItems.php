...
class TestItems extends Model
{
    ...
    public function getTtItemsAttribute()  {
        ...
        $qb_items = DB::table('test_items')
            ...
            ->where('question_type', $this->question_type)->where('test_id', $this->test_id)
        ...
        if(!empty($qb_items)) {
            foreach($qb_items as $qk=>$qitem) {
                ...
                if(in_array($qitem->question_type_id, $arr)) {
                    if(!empty($qitem->answer)) {
                        // faster solution
                        $qitem->answer = str_replace("\xc2\xa0", ' ', $qitem->answer);

                        // more flexible solution
                        $qitem->answer = preg_replace('/\xc2\xa0/', ' ', $qitem->answer);
                    }
                    ...
                    }  else if($qitem->question_type_id == 18) {
                        if(!empty($qitem->answer)) {
                            $qitem->arrange = CommonController::splitwords($qitem->answer); 
                        }   else {
                            $qitem->arrange = [];
                        }
                    } 
