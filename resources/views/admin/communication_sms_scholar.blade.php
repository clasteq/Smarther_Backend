                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="template" name="template" required>
                                            ...
                                            @foreach ($get_template as $template)
                                                <option value="{{ $template->id }}" data-content="{{ $template->content }}" data-is_name_replace="{{ $template->is_name_replace }}" data-is_name_replace_var="{{ $template->is_name_replace_var }}">{{ $template->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="position-relative">
                                        ...
                                        <div class="form-group border p-3">
                                            <div class="custom-control custom-radio custom-control-inline">
                                                ...
                                                <label class="custom-control-label" for="smart_yes">Yes ( Send SMS only to Non-App scholars )</label>
                                            </div>
                                            <div class="custom-control custom-radio custom-control-inline">
...
<script>
     $(function() {
        ...
        document.getElementById('template').addEventListener('change', function() {
            ...
            var is_name_replace = selectedOption.getAttribute('data-is_name_replace');
            var is_name_replace_var = selectedOption.getAttribute('data-is_name_replace_var');
            // Parse the content to replace '#' symbols with input boxes
            var parsedContent = parseContent(content, is_name_replace, is_name_replace_var);
        ...
        function parseContent(content, is_name_replace, is_name_replace_var) {
            ...
            /*var mydata = '';
            var content1 = content.split('#');  var place = 'Input'; 
            $(content1).each(function(index) {
                var ix = parseInt(index) + 1;  var replace_var = '##var'+ix+'##';
                /*if(is_name_replace == 1) {
                    var replace_var = '##var'+ix+'##';
                    if(is_name_replace_var == replace_var) {
                        place = 'parent name replaced here';
                    } else {
                        place = 'Input';
                    } 
                }* /
                mydata = content1.join('<input type="text" class="form-control" style="width: 200px;" placeholder="'+replace_var+'" name="vars[]" id="var_'+index+'" >'); 
            });
                console.log( mydata)  
            return mydata;*/
        }
