...
@section('master_posts_staff', 'active')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 style="font-size:20px;" class="card-title">Create Post for Staffs
                        </h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ url('admin/post_staff_message') }}" method="post" id="post_communication"
                                            ...
                                            <div class="option-container mr-3">
                                                <label class="btn btn-outline-primary">
                                                    <input data-toggle="modal" data-target="#exampleModalCenter" type="radio"
                                                        name="post_type" autocomplete="off" value="1" > Class Teacher
                                                </label>  
                                            </div>
                                            <div class="option-container mr-3">
                                                <label class="btn btn-outline-primary">
                                                    <input data-toggle="modal" data-target="#exampleModalCenter3" type="radio"
                                                        name="post_type" autocomplete="off" value="2" > Role
                                                </label>  
                                            </div>
                                            <div class="option-container mr-3">
                                                <label class="btn btn-outline-primary">
                                                    <input data-toggle="modal" data-target="#exampleModalCenter4" type="radio"
                                                        name="post_type" autocomplete="off" value="5" > Department
                                                </label>  
                                            </div>
                                            <div class="option-container mr-3">
                                                <label class="btn btn-outline-primary">
                                            ...
                                            <div class="option-container mr-3">
                                                <label class="btn btn-outline-primary">
                                                    <input data-toggle="modal" data-target="#exampleModalCenter1" type="radio"
                                                        name="post_type" autocomplete="off" value="6">
                                                    Specific Staffs
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="scrollable-form exampleModalCenterscroll">
                                            ...
                                            @foreach($get_sections as $section)
                                                <div class="sectionItem">
                                                    ...
                                                    <label for="section_{{$section->id}}">{{$section->is_class_name}}-{{$section->section_name}}-
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="scrollable-form exampleModalCenterscroll1">
                                                @foreach($get_staff as $staff)
                                                    <div class="staffItem">
                                                        <input type="checkbox" id="student_{{$staff->user_id}}" name="student_post[]" value="{{$staff->user_id}}">
                                                        <label for="student_{{$staff->id}}">{{$staff->name}}-({{$staff->emp_no}})</label><br>
                                                    </div>
                                                @endforeach  
...
<script>
            ...
            if(response.status == 1){
                var html = '';
                 $.each(response.data,function(index1, val1){
                    html += '<div class="staffItem"> <input type="checkbox" id="student_'+val1.id+'" name="student_post[]" value="'+val1.id+'"> <label for="student_'+val1.id+'">'+val1.name+'- '+val1.emp_no +'</label><br> </div>';
                 });
    ...
    function loadspecificstaffs(section_id,class_id) {
        var itemClass = '.staffItem'; 
        $('.staffItem').hide();
        if(section_id > 0) {
            $('.staffItem[data-section_id='+section_id+']').show()
        }   else if(class_id > 0) {
            $('.staffItem[data-class_id='+class_id+']').show()
        }   else {
            $('.staffItem').show();
        }
    }
...
<script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Save the initial state when the modal is opened
            $('#exampleModalCenter4').on('shown.bs.modal', function () {
                initialCheckboxState = [];
                        ...
                        if (response.status == 0) {
                            ...
                            if (response.status == 1) {
                                ...
                                swal({
                                   ...
                                   function(){ 
                                       location.href = "{{URL('/')}}/admin/posts_staff";
                                   }
                                );
