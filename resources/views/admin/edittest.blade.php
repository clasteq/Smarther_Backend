                            ...
                            <div class="card-body">
                                ...
                                <div class="row"><div class="col-md-12">
                                    ...
                                    <div class="row">
                                        ...
                                        <input type="hidden" name="class_id" id="class_id" value="{{$test['class_id']}}">
                                        <input type="hidden" name="subject_id" id="subject_id" value="{{$test['subject_id'] }}">
                                        <input type="hidden" name="term_id" id="term_id" value="{{$test['term_id'] }}">

                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Test Start Date</label>
                                            <input class="col-md-6 date_range_filter date form-control" required type="text" autocomplete="off" name="from_date" id="datepicker_from" value="{{$test['from_date']}}" />
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Last Date for Submission</label>
                                            <input class="col-md-6 date_range_filter date form-control" required type="text" autocomplete="off" name="to_date" id="datepicker_to" value="{{$test['to_date']}}" />
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Class</label>
                                            <div class="form-line"> {{$test['class_name'] }}
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Subject</label>
                                            <div class="form-line"> {{$test['subject_name'] }}
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Sections</label>
                                            <div class="form-line"> {{$test['is_section_names'] }}
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Term</label>
                                            <div class="form-line"> {{$test['term_name'] }}
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Test name</label>
                                            <?php $chaptername = ''; $qb_ids = ''; $chapter_ids = '';
                                            if(!empty($qbank) && count($qbank)>0){
                                                foreach($qbank as $qid=>$qb) {
                                                    $chaptername .= $qb->chaptername .' ';
                                                    $qb_ids .= $qb->id .',';
                                                    $chapter_ids .= $qb->chapter_id .',';
                                                }
                                            }

                                            $test_name = $chaptername; //date('Y-m-d').' '.?>
                                            <div class="form-line">
                                                <input type="hidden" name="qb_ids" id="qb_ids" value="{{$qb_ids}}">
                                                <input type="hidden" name="chapter_ids" id="chapter_ids" value="{{$chapter_ids}}">
                                         <input class="col-md-8 form-control" type="text" name="test_name" id="test_name" value="{{$test['test_name']}}" required style="width:100%;">
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Test Mark</label>
                                            <div class="form-line">
                                            <input class="col-md-6 form-control" required type="text" autocomplete="off" name="test_mark" id="test_mark"  onkeypress="return isNumber(event)" onkeyup="floatmark();"  value="{{$test['test_mark']}}" /> 
                                            </div>
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">Test Time (in minutes)</label>
                                            <div class="form-line">
                                            <input class="col-md-6 form-control" required type="text"  name="test_time" id="test_time" value="{{$test['test_time']}}" onkeypress="return isNumber(event)"  /> 
                                            </div>
                                        </div>
                                        {{-- <div class="form-group form-float float-left col-md-6">
                                        ...
                                        <div class="form-group form-float float-right col-md-12 position-fixed-center" id="floattotalmarks">Total Test Mark: {{$test['test_mark']}}
                                        </div>
                                        <div class="form-group form-float float-right col-md-12 position-fixed-center" id="floatenteredmarks" style="top:60% !important;">Total Entered Mark: {{$test['test_mark']}} 
                                        </div>
                                    </div>
                                    <hr>
                                    ...
                                    @if(!empty($items) && count($items)>0) 
                                        ...
                                        ?>
                                           ...
                                           <input class="newopration_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}} operation" type="checkbox" data-qbid="{{$qtype['question_bank_id']}}"  data-qtyid="{{$question_type}}" data-qtype="{{$qtype['question_type']}}"
                                                 name="question_type[]" id="question_type" value="{{$question_type}}"> <label class="form-label color-blue"> {{$qtype['question_type']}}
                                               </label>
                                            </div>
                                            @php($total = 0)
                                            @foreach($qtype['qb_items'] as $qk=>$item1) 
                                            @foreach($item1 as $items) 
                                                @php($test_mark = $items['test_mark']) 
                                                @if(!empty($test_mark))  
                                                    @php($total += $test_mark)
                                                @endif 
                                            @endforeach
                                            ...
                                            <div class="form-group form-float float-left col-md-9">
                                                ...
                                                </label> <input type="text" class="tot_mark total_markk_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}} col-md-2" id = "tot_mark_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}}"  placeholder="Total" data-qbid="{{$qtype['question_bank_id']}}"  data-qtyid="{{$qtype['question_type_id']}}"  data-qtype="{{$qtype['question_type']}}" name="tot_mark" value="{{$total}}" onkeyup="floatenteredmarks();">
                                            </div>
                                            ...
                                            <?php $item = $items['qb_items']; $test_mark = $items['test_mark']; 
                                                $checked = '';
                                                if(!empty($test_mark)) {
                                                    $checked = ' checked ';
                                                } 
                                                ...
                                                <div class="row">
                                                    ...
                                                    @if($qname != $items['qb_name']) 
                                                        @php($qname = $items['qb_name'])
                                                        <label class="form-label">{{$items['qb_name']}} {{$item->chaptername}}</label>
                                                    @endif
                                                    </div>
                                                    <div class="form-group form-float float-left col-md-2"> 
                                                        ...
                                                        id="question_item_id" value="{{$item->id}}" {{$checked}}>
                                                        <input class="newmarks_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}} newmm_{{$item->id}}"  data-qbid="{{$qtype['question_bank_id']}}"  data-qtyid="{{$qtype['question_type_id']}}"  type="text" name="marks[{{$item->id}}]" id="marks_{{$item->id}}" style="width: 40%;" min="1" maxlength="3" max="200" placeholder="mark" value="{{$test_mark}}">
                                                    </div>
...
@section('scripts')
      <script>
        function floatenteredmarks() {
            var tot = 0;
            $('.tot_mark').each(function(){
                var tt = parseFloat($(this).val());
                if(tt > 0){
                    tot += tt;
                }
            });
            $('#floatenteredmarks').html('Total Entered Mark: '+tot);
        }

        function floatmark() {
            $('#floattotalmarks').html('Total Test Mark: '+$('#test_mark').val());
        }  
        $(function() {
