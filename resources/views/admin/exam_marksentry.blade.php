                        ...
                        <div class="row"> 
                            ...
                            <div class=" col-md-3">
                                ...
                                <div class="form-line"> <!-- loadClassSubjects(this.value); -->
                                    <select class="form-control" name="section_id" id="section_dropdown" required 
                                        onchange="loadstudents(this.value,class_id.value);loadClassExaminations(class_id.value);">
                            ...
                            <div class="col-md-2">
                                ...
                                <button type="submit" class="btn signupBtn"
                                    ...
                                    onclick="loadexammarksentry()">Submit </button>
                            </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function loadexammarksentry() {
            ...
            var request = $.ajax({
                type: 'post',
                url: " {{ URL::to('admin/load/exam_marksentry') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {
        ...
        function updateStudentMarkEntry() {
            ...
            if(error == 0) {
                $("#submitfull").text('Processing..'); 
                $("#submitfull").prop('disabled', true);
                ...
                request.done(function(response) {
                    ...
                    $("#submitfull").text('Update'); 
                    $("#submitfull").prop('disabled', false);
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $.each(res.exams, function(key, value) {
                      ...
                      var exam_name = value.exam_name;
                      if (value.exam_name != null && value.exam_name != '') {
                        exam_name = value.exam_name;
                      } else {
                        exam_name = value.exname;
                      }
                        $("#exam_id").append('<option value="' + value
                            .id + '" '+selected+' data-startdate="'+value.monthyear+'">' + exam_name + ' - ' +value.class_name +' - ' +value.section_name + '</option>');
                    });
                }
            });
        }
