...
// foreach ($classes as $class) {
    // foreach ($periods as $key => $periodtiming){
        // print_r($class->is_mapped_subjects['id']);

        // foreach ($class->is_mapped_subjects as $value) {
        # code...
    // }
    # code..
    // }
...
@if(!empty($classes))
    @foreach ($classes as $ck => $class) <?php //echo "<pre>"; print_r($classes_arr[$ck]['subjects']);  ?>
                ...
                <tr>
                   ...
                   <td>
                    {{-- <input type="text" id="an_chk" data-key="{{$key}}"> --}}
                     <select class="form-control course_id" style="width:100% !important;" name="subject_id[{{$class->id}}][{{$sk}}][{{$periodtiming}}]"
                                >
                                <option value="0">Select Subject</option>
                               <!-- if(isset($class->is_mapped_subjects)) 
                                 foreach ($class->is_mapped_subjects as $subject) -->
                            ...
                            &nbsp;&nbsp;<?php if(!isset($timetable[$class->id][$periodtiming]) && !isset($timetable[$class->id])){?> <input type="radio" name="session[{{$class->id}}][{{$sk}}][{{$periodtiming}}]"  value="fn" @if($session == 'fn') checked @endif>FN &nbsp;&nbsp;<input type="radio"  name="session[{{$class->id}}][{{$sk}}][{{$periodtiming}}]" value="an" @if($session == 'an') checked @endif>AN <?php } ?>
                            <?php  
                        if(isset($timetable[$class->id]) && isset($timetable[$class->id][$periodtiming])) {
                       if(in_array($timetable[$class->id][$periodtiming],$fn_section)) {
                                     $checked = ' checked ';
                                     ?>
                                     <!-- <input type="radio" {{$checked}}  name="session[{{$class->id}}][{{$periodtiming}}]" value="fn">FN<?php } else{?><input type="radio" name="session[{{$class->id}}][{{$periodtiming}}]" value="fn">FN<?php
                                     } if(in_array($timetable[$class->id][$periodtiming],$an_section)){ $an_checked = "checked"; ?>&nbsp;&nbsp;<input type="radio" {{$an_checked}}  name="session[{{$class->id}}][{{$periodtiming}}]" value="an">AN<?php  } else{?><input type="radio" name="session[{{$class->id}}][{{$periodtiming}}]" value="an">AN -->
                         <?php }  } ?>
                            <br>
                            <?php $syllabus = '';
                                if(isset($examsarr['exam_session_structure'][$class->id][$sk][$periodtiming]))  { 
                                $syllabus = $examsarr['exam_session_structure'][$class->id][$sk][$periodtiming]['syllabus']; }?>
                            <textarea class="form-control" name="syllabus[{{$class->id}}][{{$sk}}][{{$periodtiming}}]" maxlength="255">{{$syllabus}}</textarea>
                        </td>
                     @endforeach
                </tr>
            @endforeach
