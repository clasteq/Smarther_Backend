...
?>
  ...
  <tbody style="text-align: center;">
      ...
      foreach( $c as $key=>$value){
          $total_marks = $totalmarks; $marks = $remarks = $grade = '';
          ...
          <tr id="{{ $student['id'] }}" class="{{ $student['id'] }}"> <th scope="row">{{$student['name']}}</th> <th scope="row">{{$student['admission_no']}}</th>
            <th scope="row">{{$value}}</th>
               ...
               <td style="display:none;"><input type="text" name="grade[{{ $key }}]" id="grade_{{ $key }}"
                              class="form-control" style="padding: 18px 22px !important;" value="{{ $grade }}"
                              minlength="1" maxlength="50"></td>
               <td><button type="submit" name="submit[{{ $key }}]" id="submit_{{ $key }}" data-key="{{ $key }}" 
                  data-name="{{$value}}" data-student="{{$student['name']}}" class="btn submit" style="background:#ffc107;border-radius: 6px;padding: 8px 13px;color:#fff;" onclick="updateMarkEntry({{ $student['id'] }},{{$key}})">Update </button></td> 
          </tr>
          <?php 
      }
      ?> 
    ...
    @endforeach 
    <tr> <td colspan="5"> <button type="button" name="submitfull" id="submitfull"
                          class="btn submitfull" style="background:#ffc107;border-radius: 6px;padding: 8px 13px;color:#fff;"
                          onclick="updateStudentMarkEntry({{ $student['id'] }})">Update </button> </td></tr>
  </tbody>
  @else 
