                ...
                <div class="card-header">
                  ...
                  <br><br> 
                       ...
                       <div class="col-md-12" id="attendanceentries">
                            @include('admin.load_students_daily_attendance')
                        </div>
                </div> 
              </div>
            </div>
...
// });
    ...
    function mv(obj, studentid) {
        var id = $(obj).attr('id');
        var tbl = $(obj).parents(".users-list").attr('id'); 
        var tr = $('div#lisection_'+studentid).remove().clone();
 
        if(tbl == 'presentlist') { 
            tr.find("#attendance_type").val("a"); 
            $("#absentlist").append(tr); 
        }   else {  
            tr.find("#attendance_type").val("p"); 
            $("#presentlist").append(tr); 
        }

        $('.present_section').prop('checked','checked')
    }


    function movefn(mode) {
        $('input[data-fn]').each(function( index ) { 
            var studentid = $(this).val();
            var fn = $(this).data('fn');
            var an = $(this).data('an');
            var tbl = $(this).parents(".users-list").attr('id'); 
