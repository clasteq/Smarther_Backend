...
<tr>
    <?php 
        ...
        // exit;?>
    <th scope="row">{{$i}}</th>
    @if(isset($player['teacherattendance']) && !empty($player['teacherattendance']) && is_array($player['teacherattendance']))
        @php($day = 'day_'.$i)
        @if($player['teacherattendance'][$day] == 1)
        <td  id="{{$player['id']}}_{{$i}}"><p class="greencherck"><i class="fa fa-check" aria-hidden="true"></i> Present</p></td>
        @elseif($player['teacherattendance'][$day] == 2)
