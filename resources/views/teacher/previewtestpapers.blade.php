...
<?php  
$breadcrumb = [['url'=>URL('/teacher/home'), 'name'=>'Home', 'active'=>''], ['url'=>URL('/teacher/testlistpapers'), 'name'=>'Tests', 'active'=>''], ['url'=>'#', 'name'=>'View Test Papers', 'active'=>'active'] ];
?>
@section('content')
                                ...
                                <div class="row">
                                    ...
                                    @if(!empty($qb['new_test_items']) && count($qb['new_test_items'])>0)
                                        @foreach($qb['new_test_items'] as $no=>$papers) 
                                            ...
                                            <button onclick="ExportToDoc('exportContent{{$no}}','{{$testname}}');" class="btn btn-info waves-effect exportContent" data-id="exportContent{{$no}}" data-no="{{$no+1}}">{{$no+1}}. Export as .pdf</button>
                                            <div class="row col-md-9"  id="exportContent{{$no}}">
                                            <table style="width:98%">
                                                <tbody>
                                                    <tr><td colspan="2" align="center">{{$qb['term_name']}}</td></tr>
...
@section('scripts') 
    ...
    <script type="text/javascript">
        $("body").on("click", ".exportContent", function () {
            var $id = $(this).data('id');
            var $no = $(this).data('no'); 
            html2canvas($('#'+$id)[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download($no+".pdf");
                }
            });
        });
    </script>
