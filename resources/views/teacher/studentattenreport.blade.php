...
@section('master_studentsatten', 'active')
                        ...
                        <div class="row"> 
                            <div class="col-md-4">
                                ...
                                <input type="month" class="form-control"
                                    ...
                                    name="monthyear" id="monthyear" value="{{ $monthyear }}" minlength="4" maxlength="7">
                            </div>
                            <div class="col-md-4">
                    ...
                    <div style="width: 100%; overflow-x: scroll; padding-left: -10px;">
                        <div class="table-responsicve">
                            <table class="table table-striped table-bordered tblcountries" id="attendanceentries">
                              @include('teacher.loadstudentsattendance')
                            </table>
                        </div>
                    </div>
                  </div>
...
@section('scripts')
    <script type="text/javascript">
        $(function() {
            ...
            loadClassSection(class_id);
        });

        function putattendance(studentid, mode, day,session,obj) {
            var monthyear = $('#monthyear').val();
            var class_id = $('#class_id').val();
            var section_id = $('#section_dropdown').val();
            var request = $.ajax({
                type: 'post',
                url: " {{ URL::to('teacher/update/studentattendancerep') }}",
                headers: {
        ...
        function loadStudentattendance() {
            ...
            var request = $.ajax({
                type: 'post',
                url: " {{ URL::to('teacher/load/studentattendancerep') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {
        ...
        function excelFunction(slot) {
            ...
            $.ajax({ 
                "url":"{{URL('/')}}/teacher/attendencereport/excel",    
                type: 'get',
                data: {
                    monthyear: monthyear,
                    slot: slot
