...
<section class="content">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h4 class="panel-title"> Add Test
                                </h4>
                                <div class="row"><div class="col-md-12">
                                    <form name="frm_questionbank" id="frm_questionbank" method="post" action="{{url('/teacher/save/qbtest')}}">
                                    {{csrf_field()}}
                                    <div class="row">
                                        ...
                                        <div class="form-group form-float float-left col-md-6">
                                            ...
                                            <input class="col-md-6 date_range_filter date form-control" type="text" autocomplete="off" name="from_date" id="datepicker_from"  />
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
                                            <label class="form-label">To</label>
                                            <input class="col-md-6 date_range_filter date form-control" type="text" autocomplete="off" name="to_date" id="datepicker_to"  />
                                        </div>
                                        <div class="form-group form-float float-left col-md-6">
 ...
 ?>
                                            ...
                                            <input class="newopration_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}} operation" type="checkbox" data-qbid="{{$qtype['question_bank_id']}}"  data-qtyid="{{$question_type}}"
                                             ...
                                             <div class="form-group form-float float-left col-md-9">
                                                 ...
                                                 </label> <input type="text" class="tot_mark total_markk_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}} col-md-2" id = "tot_mark_{{$qtype['question_bank_id']}}_{{$qtype['question_type_id']}}"  placeholder="Total" data-qbid="{{$qtype['question_bank_id']}}"  data-qtyid="{{$qtype['question_type_id']}}" name="tot_mark" >
                                             </div>
...
@section('scripts')
 ...
 $('.operation').click(function() {
     ...
     var qtyid =  $(this).data('qtyid');
       ...
       $.ajax({
                url: "{{ url('teacher/fetch-questions') }}",
                type: "POST",
                data: {
...
var qtyid =  $(this).data('qtyid');
 ...
 var questions
 var  count=0 ;
 jQuery.each(jQuery('.questions_'+qbid+'_'+qtyid+':checked'), function() {
     count++;
...
jQuery.each(jQuery('.questions_'+qbid+'_'+qtyid+':not(:checked)'), function() {
     count++;
...
var checked_val = this.value;
      $.ajax({
             url: "{{ url('teacher/fetch-questions-type') }}",
             type: "POST",
             data: {
                 questions: checked_val,
                 qbid:qbid,
             ...
             success: function(res) {
            $.each(res.questions, function(key, value) {
                $("input.newopration_"+qbid+'_'+qtyid+"[value='" + value.question_type_id + "']").prop('checked', true);
             });
     ...
     $('.newmarks_'+qbid+'_'+qtyid).val('');
      $.ajax({
             ...
             success: function(res) {
           $.each(res.questions, function(key, value) {
            $("input.newopration_"+qbid+'_'+qtyid+"[value='" + value.question_type_id + "']").prop('checked', false);
             });
             $('.total_markk_'+qbid+'_'+qtyid).val('');
...
$(".tot_mark").keyup(function(){
 ...
 jQuery.each(jQuery('.questions_'+qbid+'_'+qtyid+':checked'), function() {
  ...
  var new_length = answerss.length;
     $('.questions_'+qbid+'_'+qtyid+':checked').each(function() {
